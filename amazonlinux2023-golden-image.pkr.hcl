...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "# Check if AWS CLI is already installed (Amazon Linux 2023 includes AWS CLI v2)",
      "if command -v aws &> /dev/null; then",
      "  echo 'AWS CLI already installed:'",
      "  aws --version",
      "else",
      "  echo 'AWS CLI not found. Attempting to install...'",
      "  # Try to download from S3 via VPC endpoint (if available)",
      "  # Otherwise, skip installation as AWS CLI may not be needed for build",
      "  if curl --connect-timeout 10 --max-time 30 \"https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip\" -o \"/tmp/awscliv2.zip\" 2>/dev/null; then",
      "    echo 'Downloaded AWS CLI installer'",
      "    unzip -q /tmp/awscliv2.zip -d /tmp",
      "    sudo /tmp/aws/install",
      "    rm -rf /tmp/aws /tmp/awscliv2.zip",
      "  elif wget --timeout=10 --tries=1 -q \"https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip\" -O \"/tmp/awscliv2.zip\" 2>/dev/null; then",
      "    echo 'Downloaded AWS CLI installer via wget'",
      "    unzip -q /tmp/awscliv2.zip -d /tmp",
      "    sudo /tmp/aws/install",
      "    rm -rf /tmp/aws /tmp/awscliv2.zip",
      "  else",
      "    echo 'Warning: Could not download AWS CLI installer (private subnet without internet access).'",
      "    echo 'AWS CLI installation skipped. If needed, ensure S3 VPC endpoint is configured or use NAT Gateway.'",
      "  fi",
      "fi"
    ]
  }
  ...
  provisioner "shell" {
    inline = [
      ...
      "  # Try installing from EPEL repository (Amazon Linux 2023 compatible)",
      "  # Enable EPEL repository if not already enabled",
      "  if ! dnf repolist enabled | grep -q epel; then",
      "    echo 'Enabling EPEL repository...'",
      "    sudo dnf install -y epel-release || echo 'Warning: EPEL repository not available'",
      "  fi",
      "  # Try installing Ansible from EPEL or Amazon Linux repositories",
      "  if sudo dnf install -y ansible 2>&1 | tee /tmp/ansible-install.log; then",
      "    echo 'Ansible installed successfully via dnf'",
      "  elif sudo dnf install -y python3-ansible 2>&1 | tee -a /tmp/ansible-install.log; then",
      "    echo 'Ansible installed successfully via python3-ansible package'",
      "  else",
      "    echo 'Warning: Ansible not available in repositories. Attempting pip install...'",
      "    echo 'Note: This requires internet access or PyPI VPC endpoint'",
      "    # Try pip with timeout and better error handling",
      "    if sudo pip3 install --timeout=60 --retries=2 ansible 2>&1 | tee -a /tmp/ansible-install.log; then",
      "      echo 'Ansible installed successfully via pip'",
      "    else",
      "      echo 'ERROR: Could not install Ansible via dnf or pip.'",
      "      echo 'Please ensure:'",
      "      echo '  1. VPC endpoints are configured for S3/EC2 (for dnf repositories)'",
      "      echo '  2. NAT Gateway is configured (for pip/PyPI access)'",
      "      echo '  3. Or pre-install Ansible in the base AMI'",
      "      cat /tmp/ansible-install.log",
      "      exit 1",
      "    fi",
      "  fi",
      "  # Verify installation",
      "  if command -v ansible &> /dev/null; then",
      "    echo 'Ansible installation verified:'",
      "    ansible --version",
      "  else",
      "    echo 'ERROR: Ansible installation failed. Build cannot continue.'",
      "    exit 1",
      "  fi",
      "fi"
    ]
